...
const DrawerNavigator = () => {
  ...
  return (
    ...
    >
      <Drawer.Screen name="বাংলা ক্যালেন্ডার" component={Home}
        options={({ navigation }) => ({
          headerStyle: {
          ...
          headerLeft: () => (
            <TouchableOpacity activeOpacity={.5} style={{ paddingLeft: 10 }} onPress={() => navigation.openDrawer()}>
              <Image
                style={{ height: 37, width: 37, backgroundColor: MyColor.primaryColor, borderRadius: 25 }}
                // source={theme === 'light' ? require('../../assets/bar-thin.png') : require('../../assets/white-bar.png')}
                source={require('../../assets/white-bar.png')}
              />
            </TouchableOpacity>
          ),
          drawerLabel: 'Home',
          drawerActiveTintColor: theme === 'light' ? MyColor.white : MyColor.fontColor,
          drawerActiveBackgroundColor: MyColor.primaryColor,
          drawerIcon: ({ focused, size }) => (
            <Icon
              name="home"
              size={size}
              color={theme === 'light' && focused ? MyColor.white : MyColor.fontColor}
            />
          ),
        })}
      />
